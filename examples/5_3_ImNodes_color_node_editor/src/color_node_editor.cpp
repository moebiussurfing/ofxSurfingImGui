	...
	namespace
	{
		...
		public:
			...
			void show()
			{
				...
				current_time_seconds = 0.001f * ofGetFrameNum();
				...
				// These are driven by the user, so we place this code before rendering the nodes
				{
					...
					const bool open_popup = ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) &&
						ImNodes::IsEditorHovered() && ofGetKeyPressed('a');
				...
				// simple workaround to avoid retrig
				static bool bTrig1 = false;
				static bool bTrig2 = false;
				static uint64_t tLast1 = 0;
				static uint64_t tLast2 = 0;
				if ((ofGetElapsedTimeMillis() - tLast1 > 200) && ofGetKeyPressed('x') && !tLast1)
				{
					tLast1 = ofGetElapsedTimeMillis();
					bTrig1 = true;
				}
				if ((ofGetElapsedTimeMillis() - tLast2 > 200) && ofGetKeyPressed('x') && !tLast2)
				{
					tLast2 = ofGetElapsedTimeMillis();
					bTrig2 = true;
				}
