...
//	ImVec2 tmp = ImVec2(0.7f, 1.1f) * ImVec2(fTime, fTime) + ImVec2(0.0f, 2.0f);
//	ImVec2 w = ImVec2(0.750f, 0.25f) * (ImVec2(0.5f, 0.5f) + ImVec2(ImCos(tmp.x), ImCos(tmp.y)) * ImVec2(0.5f, 0.5f));
//
//	// distance
//	float d = sdHorseshoe(p - ImVec2(0.0f, -0.1f), ImVec2(ImCos(t), ImSin(t)), 0.5f, w);
//
//	// coloring
//	ImVec4 col = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);// -ImVec4(0.1f, 0.4f, 0.7f, 1.0f);
//	//ImVec4 col = ImVec4(1.0f, 1.0f, 1.0f, 1.0f) - ImVec4(0.1f* ImSign(d), 0.4f* ImSign(d), 0.7f* ImSign(d), 1.0f* ImSign(d));
...
//					float const width = ImGui::GetContentRegionAvail().x;
//					ImVec2 curPos = ImGui::GetCursorScreenPos();
//					ImGui::InvisibleButton("##Zone", ImVec2(width, width) * ImVec2(0.5f, 0.5f), 0);
//
...
//					DrawColorRingEx< true >(pDrawList, curPos, ImVec2(width, width) * ImVec2(0.5f, 0.5f), thickness,
//						[fFreq](float t)
//					{
...
//							center + dir * ImVec2(0.5f * width, 0.5f * width),
