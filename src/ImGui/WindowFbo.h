...
public:
	...
	ofParameter<bool> bGui_PlotFullScreen{ "Full Screen", true };//big preview will be drawn on the full screen
...
public:
	...
	void setup()
	{
		...
		params.add(bGui_PlotFullScreen);
			...
			ImGui::Begin(bGui_Extra.getName().c_str(), (bool*)&bGui_Extra.get(), flagsw);
			{
				ImGui::Indent();
				{
					...
					if (bGui_PreviewBig)
					{
						ImGui::Indent();
						{
							ofxImGuiSurfing::AddToggleRounded(bGui_PlotFullScreen);
