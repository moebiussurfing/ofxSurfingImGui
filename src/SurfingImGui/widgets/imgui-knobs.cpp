...
namespace ImGuiKnobs {
	namespace detail {
		...
		color_set GetSecondaryColorSet() {
			...
			//auto active = ImVec4(ofColor::blueViolet);
			//auto hovered = ImVec4(ofColor::orange);
			//return {base, hovered, active};
		...
		color_set GetTrackColorSet() {
			...
			auto active = colors[ImGuiCol_FrameBgActive];
			auto hovered = colors[ImGuiCol_FrameBgHovered];
			auto base = colors[ImGuiCol_FrameBg];
	...
	bool BaseKnob(const char* label, ImGuiDataType data_type, DataType* p_value, DataType v_min, DataType v_max, float speed, const char* format, ImGuiKnobVariant variant, float size, ImGuiKnobFlags flags, int steps = 10) {
		...
		*/
		...
		case ImGuiKnobVariant_Dot: {
			...
			//more resolution for circles
			knob.draw_circle(0.85, detail::GetPrimaryColorSet(), true, 48);
			knob.draw_dot(0.12, 0.6, knob.angle, detail::GetSecondaryColorSet(), true, 32);
			break;
		}
