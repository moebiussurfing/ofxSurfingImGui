...
namespace ofxImGuiSurfing
{
    ...
    //--------------------------------------------------------------
    inline void AddTextTimecode(int seconds, std::string pre = "", bool onDrawList = false, ImVec2 pos = ImVec2(0, 0),
                                float fontScale = 1.0f)
    {
        ...
        if (onDrawList)
        {
            ...
            draw_list->AddText(ImGui::GetFont(), ImGui::GetFontSize() * fontScale, pos, IM_COL32_WHITE, temp, NULL,
                               0.0f);
        }
        else
        {
    ...
    inline void AddLabelLinkURL(const char* desc, const char* url, bool bBlink = true)
    {
        ...
        if (bDebug)
            IMGUI_SUGAR__DEBUG_POINT(ofColor::yellow);
    ...
    //--------------------------------------------------------------
    static bool AddFileSelector(const std::string& path, std::string& selected, /*int &index, */
    ...
    // creates a button for each file showing each file names
    //--------------------------------------------------------------
    static bool AddFilesPicker(const std::string& path, std::string& _selected, ofParameter<int>& _index,
                               const std::vector<std::string>& ext = {})
    {
        bool ret = false;
    ...
    //--------------------------------------------------------------
    inline bool AddPad2D(ofParameter<float>& x, ofParameter<float>& y, ImVec2 sz = ImVec2{-1, -1}, bool bFlipx = false,
                         bool bFlipy = false)
    {
        //ImGui::Spacing();
    ...
    inline void AddPlot(ofParameter<float>& parameter, ImVec2 sz/* = ImVec2(-1, -1)*/,
                        vector<SliderMarks>* marks = nullptr, bool bMinimized = false, float scale = 1.f,
                        bool bOverlayAvg = false)
    {
    ...
    inline void AddPlot(ofParameter<float>& parameter, vector<SliderMarks>* marks, bool bMinimized = false,
                        float scale = 1.f, bool bOverlayAvg = false)
    {
        ImVec2 sz = ImVec2(-1, -1);
    ...
    inline void AddVoidWidget()
    {
		...
		//TODO: workaround ugly fix
		float h = ImGui::GetStyle().FontSizeBase + ImGui::GetStyle().FramePadding.y * 2;
    ...
    // To customize label
    //--------------------------------------------------------------
    inline void AddProgressBar(const float vPrc, string text, float max = 1.0f)
    {
        const float _w100 = ImGui::GetContentRegionAvail().x;
        const float pad = 0;

        //TODO:
        // char* overlay = text.c_str();
        //if (bNoText) overlay = "";
        ...
        {
            ImGui::ProgressBar(_prc, ImVec2(_w100 - pad, 0), text.c_str());
        }
    ...
    */
    ...
    inline void AddProgressBarVertical(const char* label, const float value, const ImVec2& sz)
    {
        ...
        // Bg
        draw_list->AddRectFilled(rect_start, rect_start + rect_size, ImGui::GetColorU32(ImGuiCol_FrameBg),
                                 style.FrameRounding);
        ...
        // Border
        draw_list->AddRect(rect_start, rect_start + rect_size, ImGui::GetColorU32(ImGuiCol_Border),
                           style.FrameRounding);
    ...
    //--------------------------------------------------------------
    inline void drawOfTexture(ofTexture* tex, float& _tw, float& _th, float& posX, float& posY, float& drawW,
                              float& drawH)
    {
#define IMGUI_EX_NODE_HEADER_HEIGHT 20
        ...
        if (tex->isAllocated())
        {
            ...
            {
                // always considered vertical texture
                drawW = (tex->getWidth() * _th) / tex->getHeight();
                drawH = _th;
                posX = (_tw - drawW) / 2.0f;
                posY = 0;
    ...
    //--------------------------------------------------------------
    inline void drawWaveform(ImVec2 sz, float* data, int dataSize, float thickness, ImU32 color,
    ...
    //--------------------------------------------------------------
    inline void AddWaveform(string name, float* data, uint32_t size, bool bWindowed = true, ImVec2 sz = ImVec2(-1, -1),
                            bool bNoHeader = false, vector<SliderMarks>* marks = nullptr)
    {
        bool b = false;
        if (bWindowed)
        {
            ...
            ImGui::SetNextWindowSizeConstraints(ImVec2{100, 100 * r}, ImVec2{
    ...
    //--------------------------------------------------------------
    inline void AddFFT(string name, std::vector<float>* data, float max, bool bWindowed = true,
                       ImVec2 sz = ImVec2(-1, -1), bool bNoHeader = false, float start = 0.f, float end = 1.f)
    {
        bool bFill = true;
    ...
    //--------------------------------------------------------------
    inline void AddVU(string name, float value, bool bHorizontal = false, bool bWindowed = true,
                      ImVec2 sz = ImVec2(-1, -1), bool bNoHeader = false, float padding = 1.f, int divisions = 20)
    {
    ...
    //--------------------------------------------------------------
    inline void AddSliderBigVerticalFloating(ofParameter<float>& p, ImVec2 sz = ImVec2(-1, -1), bool bNoTitle = true,
                                             ofColor* colorGrab = nullptr, vector<SliderMarks>* marks = nullptr)
    {
        ImGui::BeginGroup();
