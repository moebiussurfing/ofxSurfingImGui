...
namespace ImGui
{
	...
	// Custom colors
	static float c1 = 0.09f;
	static float c2 = 0.1f;
	static float c3 = 0.0f;
	//IM_COL32(100, 100, 100, 255));
	static ImVec4 colorElements = { c1,c1,c1,0.95f };//elements
	static ImVec4 colorOuter = { c2,c2,c2,0.3f };//outer box
	static ImVec4 colorSelect = { 0,1,0,0.75f };//selected
	static float rounded = 0.0f;
...
//--------------------------------------------------------------
		...
		for (auto markIt = gradient->getMarks().begin(); markIt != gradient->getMarks().end(); ++markIt)
		{
			...
			if (selectedMark == mark)
			{
				draw_list->AddTriangleFilled(ImVec2(to, bar_pos.y + (height - 3)),
					...
					ImVec2(to + 4, barBottom + 1), ImGui::GetColorU32(colorSelect));

				draw_list->AddRect(ImVec2(to - 5, bar_pos.y + (height + 1)),
					ImVec2(to + 5, bar_pos.y + (height + 11)),
					ImGui::GetColorU32(colorSelect), 1.0f, 1.0f);
			}
	...
	bool GradientButton(ImGradient* gradient)
	{
		...
		bool clicked = ImGui::InvisibleButton("gradient_bar", ImVec2(maxWidth, _h));

		DrawGradientBar(gradient, widget_pos, maxWidth, _h);
	...
	bool GradientEditor(ImGradient* gradient,
		ImGradientMark* & draggingMark,
		ImGradientMark* & selectedMark, bool bEdit)
	{
		if (!gradient) return false;

		static const float GRADIENT_BAR_EDITOR_HEIGHT = 1.5 * ImGui::GetFrameHeight();
